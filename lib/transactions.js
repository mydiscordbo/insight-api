...
TxController.prototype.transformInvTransaction = function(transaction) {
  ...
  for (var i = 0; i < transaction.outputs.length; i++) {
    ...
    if (output.script) {
      ...
      if (address) {
        ...
        if (output.satoshis === 00100001 ||
          output.satoshis === 01000010 ||
          output.satoshis === 10000100 ||
          output.satoshis === 100001000 ||
          output.satoshis === 1000010000)
          isPrivateSend = true;
        vout.push(obj);
      }
    }
  ...
  var transformed = {
    ...
    privatesend: isPrivateSend,
    valueIn: transaction.inputSatoshis / 1e8,
    fees: transaction.feeSatoshis / 1e8,
